      ...
      for (const sizeKey of generateSizes) {
        ...
        if (sizeConfig.width && sizeConfig.height) {
          if (maintainAspectRatio) {
            // For landscape images (charts, graphs), prioritize width
            // For portrait images, prioritize height
            const aspectRatio = metadata.width / metadata.height
            let newWidth, newHeight

            if (aspectRatio > 1) {
              // Landscape image - fit to width, calculate height
              newWidth = Math.min(sizeConfig.width, metadata.width)
              newHeight = Math.round(newWidth / aspectRatio)
            } else {
              // Portrait image - fit to height, calculate width  
              newHeight = Math.min(sizeConfig.height, metadata.height)
              newWidth = Math.round(newHeight * aspectRatio)
            }

            // Use 'inside' to ensure the entire image fits without cropping
